...
router.post('/session', async (req,res)=>{
  try {
    ...
    const response=await fetch('https://verification.didit.me/v3/session/',{method:'POST',headers:{'x-api-key':config.diditApiKey,'Content-Type':'application/json'},body:JSON.stringify({workflow_id:config.diditKycWorkflowId,vendor_data:`robank-user-${ownedWallet}`})});
